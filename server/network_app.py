...
def del_associated_file(data, db):
    names_column_file = data['names_column_file']
    list_tables = data['tables']
    for name_table in list_tables:
        list_rows = list_tables[name_table]
        for row in list_rows:
            dir_associated_file = db.open_dir_associated_file(row, name_table, names_column_file[name_table])
...
def get_persons_for_assessment(name_db):
    ...
    for i in range(len(persons)):
        ...
        for criterion in f.mysql_castom_command("SELECT id_conf_criterion, title_criterion FROM conf_criterion"):
            ...
            if len(exist_assessment) != 0:
                if exist_assessment[0][8] == 0:
                    person["add_data"][criterion[1]] = ' ' + exist_assessment[0][7]
                elif exist_assessment[0][8] == 1:
                    person["add_data"][criterion[1]] = '$ ' + exist_assessment[0][7]
                else:
...
def send_assessments(name_db):
    ...
    # user=session[]
    f = database.FurnitureDtabase(name_db)
    id_person_user = f.mysql_castom_command(
        f"SELECT id_personal FROM users WHERE id_users = '{session['user'][0]['id_user_mydb']}'")[0][0]
