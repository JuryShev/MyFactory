    ...
    def next(self):
        ...
        if self.table_start_.value_department > 0 and self.table_start_.value_criterion > 0:
            ...
            self.MainWindowAll.GlobalstackedWidget.setCurrentIndex(
                self.MainWindowAll.GlobalstackedWidget.currentIndex() + 1)
            print(
                f"value_criterion={self.table_start_.value_criterion}\nvalue_departmen={self.table_start_.value_department} ")
        else:
...
class Table_start_(QWidget, Table_start_v2):
    ...
    def build_data_in_json(self, json_templ, id_flag):
        ...
        self.write_in_data_id(self.table_bonus_koeficient, self.bonus_koeficient, json_templ, 'bonus_koeficient',
                              id_flag)
    ...
    def start_send_data(self):
        ...
        thred_progress_bar = Worker_2(partial(self.progress_bar.proc_counter,
                                              status='on'))  # Any other args, kwargs are passed to the run function
    ...
    def thread_complete(self):
        ...
        if self.progress_bar.status_ == 'ok':
            ...
            if self.MainWindowAll != None:
                ...
                self.MainWindowAll.GlobalstackedWidget.setCurrentIndex(
                    self.MainWindowAll.GlobalstackedWidget.currentIndex() + 1)
        elif self.progress_bar.status_ == 'none_operation; none_operation;':
            self.label_error.setStyleSheet("color: rgb(16, 255, 149);")
            self.label_error.setText(_translate("Form", 'none operation;'))
        else:
...
class mywindow(QtWidgets.QMainWindow):
    ...
    def change_name(self):
        ...
        worker_2 = Worker_2(partial(self.progress_bar.proc_counter,
                                    status='on'))  # Any other args, kwargs are passed to the run function
